	...
	public void startListener(Database db) {
		...
		Main.mainWindow.getBtnContact().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ResultSet rsStudie = db.executeStatement("select naam from Opleiding");
				Object[] studieNamen = null;
				try {
					rsStudie.last();
					studieNamen = new Object[rsStudie.getRow()];
					rsStudie.beforeFirst();
					while (rsStudie.next()) {
						studieNamen[rsStudie.getRow() - 1] = rsStudie.getString("naam");
					}
				} catch (SQLException ex) {
					ex.printStackTrace();
				}
				String naam = (String) JOptionPane.showInputDialog(null, "Kies naam", "Studienaam:",
						JOptionPane.QUESTION_MESSAGE, null, studieNamen, studieNamen[0]);
				if (naam != null) {
					Main.mainWindow.getSplitPane().setRightComponent(invoer);
					ResultSet rs = db.executeStatement(
							"SELECT * from Contactpersoon where id = (select contactpersoon from Opleiding where naam ="
									+ "'" + naam + "'" + ")");
					try {
						rs.next();
						...
						invoer.getBtnOk().addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent e) {
								...
								try {
									model = new ContactModel(rs.getString("id"), invoer.getTxtEmail().getText(),
											invoer.getTxtTel().getText(), invoer.getTxtNaam().getText(),
											(String) invoer.getComGeslacht().getSelectedItem(),
					...
					public void mouseClicked(MouseEvent me) {
						if (me.getClickCount() == 2) {
							...
							if (type.equals("Model.StudentModel")) {
								...
								ResultSet tel = db
										.executeStatement("select telefoonnummer from HHS_student_tel where id = '"
												+ studentModel.getId() + "'");
								...
								ResultSet tel = db
										.executeStatement("select telefoonnummer from EXC_student_tel where id = '"
												+ excModel.getId() + "'");
								StringBuilder sb = new StringBuilder();
								try {
									while (tel.next() && !tel.isLast()) {
				...
				ActionListener zoekListener = new ActionListener() {
					...
					public void actionPerformed(ActionEvent ae) {
						...
						ResultSet rs = db.executeStatement("SELECT * FROM HHS_student WHERE achternaam LIKE'%"
								+ optiesPane.getTxtFieldAchternaam().getText() + "%' and naam LIKE '%"
								+ optiesPane.getTxtFieldVoornaam().getText() + "%'");
						ResultSet rsex = db.executeStatement("SELECT * FROM EXC_student WHERE achternaam LIKE '%"
								+ optiesPane.getTxtFieldAchternaam().getText() + "%' and voornaam LIKE '%"
								+ optiesPane.getTxtFieldVoornaam().getText() + "%'");
						try {
						...
						if (e.getStateChange() == ItemEvent.SELECTED) {
							if (e.getItem().equals("Wijzigen")) {
								...
								try {
									...
									Object land = JOptionPane.showInputDialog(null, "Kies land", "land calamiteiten",
		...
		Main.mainWindow.getBtnGegOpvragen().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				gegOpvragen.getBtnContactpersoon().addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent e) {
						ResultSet rsNaam = db.executeStatement("select naam from Opleiding");
						Object[] naam = null;
						try {
							rsNaam.last();
							naam = new Object[rsNaam.getRow()];
						...
						String opleiding = (String) JOptionPane.showInputDialog(null, "Kies Opleiding", "Opleiding",
								JOptionPane.QUESTION_MESSAGE, null, naam, naam[0]);

						ResultSet rsContactpersoon;
						if (opleiding != null) {
							rsContactpersoon = db.executeStatement(
									"Select Opleiding.naam, Contactpersoon.naam as contactpersoon, Contactpersoon.telefoonnummer from Opleiding\n"
											+ "	join Contactpersoon on Opleiding.contactpersoon = Contactpersoon.id\n"
											+ "	where Opleiding.contactpersoon in (select Contactpersoon.id from Contactpersoon) and Opleiding.naam = '"
											+ opleiding + "'");
							try {
								rsContactpersoon.next();
								JOptionPane.showMessageDialog(null,
										"Contactpersoon voor " + opleiding + ": \n"
												+ rsContactpersoon.getString("contactpersoon") + ": "
												+ rsContactpersoon.getString("telefoonnummer"));
							} catch (SQLException ex) {
								Logger.getLogger(Actions.class.getName()).log(Level.SEVERE, null, ex);
							}
						}
